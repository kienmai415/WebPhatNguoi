...
public class UpdateReportStatusServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean updated = reportsDao.updateReportStatus(reportId, newStatus);

        if (updated) {
            response.sendRedirect("police.jsp?status=Pending");
        } else {
            response.sendRedirect("police.jsp?status=Pending&error=1");
        }
    }
